...
import { Instagram, Music2, Youtube, Loader2 } from "lucide-react";
...
export default function Home() {
  ...
  // AUTOMAÇÃO: Detecta a plataforma pelo link
  useEffect(() => {
        ...
        <div className="fixed inset-0 z-[100] flex flex-col items-center justify-center bg-white/80 backdrop-blur-sm">
          <div className="flex flex-col items-center gap-4">
            <div className="relative flex items-center justify-center">
              <Loader2 className="animate-spin absolute text-pink-200 mb-4" size={180} />
              <img
                src="https://s5.ezgif.com/tmp/ezgif-5076eea013fac75c.gif"
                alt="Loading"
                className="max-w-[90px]"
              />
            </div>
            <div className="flex flex-col items-center text-center px-4">
              <p className="text-4xl font-semibold text-pink-400 font-[cursive] animate-pulse">
                Processando seu vídeo...
              </p>
              <p className="text-pink-300 text-lg mt-2">(੭˃ᴗ˂)੭ ✨ Identificamos que é um link do {selected.toUpperCase()}!</p>
            </div>
          </div>
        </div>
      )}
      ...
      <main className="mx-auto mb-8">
        ...
        <div className="flex items-center text-pink-300 p-4 rounded-lg h-70 backdrop-blur-xs z-50 relative bg-pink-300/10 font-semibold justify-center flex-col gap-2">
          <div className="border-2 flex items-center justify-center flex-col p-6 gap-2 rounded-lg border-pink-300 h-full">
            <p className="text-4xl">(˶˃ ᵕ ˂˶)</p>
            <h1 className="text-4xl font-[cursive]">Conversor de arquivos</h1>
          </div>
        </div>
      </main>

      <div className="rounded-xl flex z-50 mx-auto max-md:w-[90%] w-3xl shadow-lg border-2 border-pink-200 overflow-hidden">
        <input
          value={url}
          onChange={(e) => setUrl(e.target.value)}
          placeholder={placeholders[selected]}
          disabled={loading}
          className="focus:ring-0 outline-none w-full text-[#A59389] px-4 bg-white py-4 text-xl transition-all"
        />
        <button
          onClick={handleDownload}
          disabled={loading}
          className={`h-full bg-pink-300 md:w-40 w-32 text-xl font-medium text-white cursor-pointer transition-colors flex items-center justify-center ${loading ? 'opacity-50 cursor-not-allowed' : ''}`}
        >
          {loading ? <Loader2 className="animate-spin" /> : "Baixar"}
        </button>
      </div>

      {/* Navegação que muda sozinha conforme o link */}
      <nav className="flex max-md:flex-col gap-8 items-center mt-12 justify-around w-[90%]">
        <div
          onClick={() => !loading && handleTabChange("youtube")}
          className={`text-pink-300 border text-lg w-full p-6 rounded-lg backdrop-blur-xs z-50 relative text-center cursor-pointer flex flex-col items-center gap-2 transition-all duration-500 ${selected === "youtube"
            ? "bg-pink-100 border-pink-400 scale-110 shadow-xl ring-4 ring-pink-200/50"
            : "bg-white border-gray-300 opacity-60 scale-95"
            }`}
        >
          <Youtube size={32} />
          <div>
        ...
        <div
          onClick={() => !loading && handleTabChange("tiktok")}
          className={`text-pink-300 border text-lg w-full p-6 rounded-lg backdrop-blur-xs z-50 relative text-center cursor-pointer flex flex-col items-center gap-2 transition-all duration-500 ${selected === "tiktok"
            ? "bg-pink-100 border-pink-400 scale-110 shadow-xl ring-4 ring-pink-200/50"
            : "bg-white border-gray-300 opacity-60 scale-95"
            }`}
        >
        ...
        <div
          onClick={() => !loading && handleTabChange("instagram")}
          className={`text-pink-300 border text-lg w-full p-6 rounded-lg backdrop-blur-xs z-50 relative text-center cursor-pointer flex flex-col items-center gap-2 transition-all duration-500 ${selected === "instagram"
            ? "bg-pink-100 border-pink-400 scale-110 shadow-xl ring-4 ring-pink-200/50"
            : "bg-white border-gray-300 opacity-60 scale-95"
            }`}
        >
